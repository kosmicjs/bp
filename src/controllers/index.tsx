...
export const get: Middleware = async function (ctx) {
  await ctx.renderRaw(
    <Layout>
      <div class="d-flex flex-column align-items-md-center mb-5">
        {['Koa', 'HTMX', 'Postgres'].map((tech, idx) => (
          <>
            <span class="h3 text-bg-info p-2">{tech}</span>
            {idx === 2 ? null : <span class="h4">+</span>}
          </>
        ))}
      </div>
